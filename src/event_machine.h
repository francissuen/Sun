...
{
public:
    using Handler = std::function<void(const TEvent &, const std::shared_ptr<void> &)>;
    struct EventData
    {
        TEvent ev;
...
public:
    void Register(const TEvent & ev, const THandlerID & handler_id, const Handler & handler)
    {
    ...
    void PostEvent(const TEvent & ev)
    {
        event_data_queue_.push_back(ev);
    }
...
private:
    std::unordered_map<TEvent, std::unordered_map<THandlerID, Handler>> handlers_;
