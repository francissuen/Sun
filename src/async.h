...
class Async<TRet(TArgs...)> {
 ...
 private:
  ...
  struct HelperImpl<false, TDummy> {
    ...
    static void SetThreadIdx(const std::size_t thread_idx,
                             typename Package::Args& args) {
      FS_SUN_UNUSED(thread_idx, args);
    }
  };
 ...
 public:
  ...
  std::size_t GetThreadCount() const { return threads_.size(); }

 private:
  void ThreadFunc(const std::size_t thread_idx) {
    std::unique_lock<std::mutex> lck(pkg_buffer_mtx_);
    for (;;) {
      if (quit_) break;
 ...
 private:
  unsigned int finished_thread_count_{0u};
...
class AsyncBatched<TRet(TArgs...)> {
 ...
 public:
  ...
  std::size_t GetThreadCount() const { return worker_.GetThreadCount(); }

 private:
  void Commiter(std::vector<Package> pkg_cache) {
    static const std::size_t thread_count = worker_.GetThreadCount();
    const std::size_t q = pkg_cache.size() / thread_count;
    const std::size_t r = pkg_cache.size() % thread_count;
